...
export class DriverComponent implements OnInit {
 ...
 public newSal:number;
  
  constructor(private deliverymanService:DriverService,private https : HttpClient) { 
    this.deliverymans=[];
    this.driverM=[];
    this.newSal=0;
...
getSal( idDriver : number){


  return this.https.get<number>("http://localhost:8092/SpringMVC/servlet/setSal/"+ idDriver).subscribe(res=>{
    this.newSal = res;

   
console.log(this.newSal);
